...
3. **三维一体安全下载机制**
   ...
   * **时效性分享链接**：支持生成 HMAC 加密预签名下载链接，有效期可选 10 分钟 / 1 小时 / 6 小时 / 1 天 / 7 天，过期自动失效。
...
5. **在线预览与命令面板**
    * 图片 / 音视频 / PDF / Markdown / 代码可在面板内直接预览，音视频走 HTTP Range 分片。
    * 宫格视图对图片显示缩略图；文件夹内若有 `README.md` 会自动渲染。
    * `⌘K` 命令面板、类型筛选、列排序、相对时间、文件夹拖拽上传。
    * 时效分享支持 10 分钟 / 1 小时 / 6 小时 / 1 天 / 7 天，并一键复制 wget / curl。

6. **多 VPS 证书推送与共享分发**
   * 控制台新增 **SSL 证书看板**，按域名展示推送时间与文件，并复制下游拉取命令。
...
* `GET /api/share?key=<key>&ttl=<seconds>`：生成预签名分享链接。`ttl` 可选 `600` / `3600` / `21600` / `86400` / `604800`（10 分钟～7 天），默认 10 分钟。
* `GET /api/certs`：按域名聚合 `certs/` 目录，供控制台证书看板使用。
* `GET /api/download`：流式下载，支持 `Range`、`HEAD`、`inline=1` 在线预览。
